...
export class SubscriptionsPaymentController {
  constructor(
    private subscriptionsPaymentService: SubscriptionsPaymentService,
  ) {}
  ...
  async stripeCheckout(
    ...
    return this.subscriptionsPaymentService.stripeCheckout(
      subscriptionId,
      interval,
    );
  }
  ...
  async stripeWebhook(
    ...
    if (!environment.STRIPE_WEBHOOK_SECRET_SUBSCRIPTIONS) {
      throw new BadRequestException(
        'STRIPE_WEBHOOK_SECRET_SUBSCRIPTIONS no configurado',
      );
    }
