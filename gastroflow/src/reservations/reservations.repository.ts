...
export class ReservationsRepository{
    ...
    async AllReservations(restaurantId: string){
        ...
        if (!reservations.length) throw new NotFoundException('No se encontraron reservas para este restaurante');
        return reservations;
    } 
    ...
    const tableNotAvailable = await this.reservationsRepository.findOne({
        where: {
            ...
            start_time: LessThan(new Date(new Date(start_time).getTime() + (2 * 60 + 15) * 60 * 1000)),
...
async cancelReservation(restaurantId: string, reservationId: string) {
    ...
    if (reservation.status === 'CANCELADO') throw new BadRequestException('La reserva ya está cancelada');
    
    reservation.status = 'CANCELADO';
    
    await this.restaurantsTableRepository.updateStatus(restaurantId, reservation.table.id, RestaurantTableStatus.AVAILABLE)
